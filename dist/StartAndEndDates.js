...
const StartAndEndDates = ({
  ...
  }, [startDateValue, endDateValue]); // boolean that has an accurate value at all times - whether both dates are invalid

  const areBothDatesInvalid = useMemo(() => !isValidDate(startDateValue) && !isValidDate(endDateValue), [startDateValue, endDateValue]); // effect on page load that sets the dates to reflect 12-1pm on the current date.
  ...
  }, [isDetailsMode, setValue, areBothDatesInvalid]); // effect that watches for user changes to 'start'.
  ...
  }, [startDateValue]); // effect that watched for user changes to 'end'.
  ...
  return /*#__PURE__*/React.createElement(GroupOfInputs, props);
};

export { StartAndEndDates };