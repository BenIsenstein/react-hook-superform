...
import { useForm } from "react-hook-form"; //import UserContext from '../../UserContext'
...
const SuperForm = ({
  ...
  if (!onSubmit) onSubmit = () => alert('No onSubmit given to <FormTemplate />'); // - - - - - - - - - - - Hooks - - - - - - - - - - -
  ...
  const [resetValues, setResetValues] = useState({}); //const userContext = useContext(UserContext)
  ...
  const resetForm = useMemo(() => () => {
    reset(resetValues);
    setViewMode('details');
  }, [reset, resetValues]); // - - - -  Conditions measuring formMode + viewMode - - - -
  ...
  useEffect(() => !openInEditView && setViewMode('details'), [openInEditView]); // Effect to conditionally bring in entry and populate fields
  ...
  useEffect(() => {
    ...
    const getDetails = async () => {
      try {
        ...
        for (let key in details) if (isDateInput(key)) details[key] = new Date(details[key]); // function to find the value of an input field, no matter how nested the name is.
        // breaks nested input names down by dot notation, but works for non-nested input values as well.


        const findValInDetails = name => name?.split('.')?.reduce((curVal, curKey) => curVal && curVal[curKey], details); // for each input, setValue + add the value to valuesForReset
        ...
        console.log(err);
        alert("There as an error loading your details. We're working on it as fast as we can."); //goHome()
      }
    };

    getDetails();
    return () => isMounted = false;
  }, [isDetailsMode, detailsUrl, isDateInput, setAreDetailsLoaded, setValue, goHome]); // Effect to conditionally control the value of 'hasBeenChanged'. 
  // It can be useful to know if the form has been edited.
  // It won't render infinitely, the logic at the end prevents it.

  useEffect(() => {
    if (!isDetailsMode) return; // store the current value the form data

    let currentValues = {};

    for (let {
      name
    } of inputs) currentValues[name] = watch(name); // see if current form data is identical to the original values


    let doEntriesMatch = Object.keys(resetValues).every(key => isDateInput(key) ? resetValues[key]?.toString() === currentValues[key]?.toString() : resetValues[key] === currentValues[key]);
  ...
  return /*#__PURE__*/React.createElement(FlexSection, _extends({
    ...
    column: true,
    fadeIn: true
  }, props), props.BeforeTemplate, /*#__PURE__*/React.createElement(FlexSection, {
    fullWidth: true,
    spaceBetween: true
  }, /*#__PURE__*/React.createElement(FormSectionTitle, {
    as: props.titleTag
  }, props.titleText), !props.displayOnly && isDetailsMode && /*#__PURE__*/React.createElement(PencilIcon, {
    onClick: isEditView ? props.editViewCancel || resetForm : () => setViewMode('edit')
  })), /*#__PURE__*/React.createElement(Form, _extends({}, props.formProps, {
    onSubmit: handleSubmit(async data => {
      setIsSubmitting(true);
      await onSubmit(data);
      setIsSubmitting(false);
    })
  }), /*#__PURE__*/React.createElement(GroupOfInputs, _extends({
    inputs: inputs
  }, modeAndView, formTools)), props.BeforeSubmitButton, isEditView && props.BeforeSubmitButtonIfEditView, (isAddMode || isEditView) && /*#__PURE__*/React.createElement(FlexSection, {
    fullWidth: true,
    marginTop1em: true
  ...
  }, isSubmitting ? "Please wait" : props.submitText || "Save"), !props.noCancelButton && /*#__PURE__*/React.createElement(Button, {
    fullWidth: true,
    type: "button",
    onClick: isEditView ? props.editViewCancel || resetForm : props.addModeCancel || goHome
  }, props.cancelText || "Cancel"))), props.AfterTemplate);
};
